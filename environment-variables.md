...
setting one never leaks into another command. Run `incus-compose <command> --help` for the full list - every flag's
...
| `INCUS_COMPOSE_IMAGE_CACHE`  | `--image-cache`  | Incus project used as image cache (default: `incus-compose-cache`); set `""` to disable      |
...
`healthd down`), not global ones.

The ic-healthd daemon reads a further set of `INCUS_COMPOSE_HEALTHD_*` variables
of its own, which incus-compose injects into the sidecar - see The ic-healthd daemon below.
...
they matter when you run the daemon yourself.
...
Note that `INCUS_COMPOSE_HEALTHD_INCUS` appears twice with two
different readers: on `up` and `healthd up` it tells _incus-compose_ what
endpoint to configure the sidecar with, and for the daemon itself it is what it dials.
...
- [Health Checking](/healthd) - the ic-healthd sidecar
